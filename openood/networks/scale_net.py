...
class ScaleNet(nn.Module):
    ...
    def get_logits_scalefactor(self, x, percentile):
        _, feature = self.backbone(x, return_feature=True)
        scale_factor = get_scalefactor(feature.view(feature.size(0), -1, 1, 1), percentile)
        feature = feature.view(feature.size(0), -1)
        logits = self.backbone.get_fc_layer()(feature)
...
def get_scalefactor(x, percentile=65):
    assert x.dim() == 4
    assert 0 <= percentile <= 100
    b, c, h, w = x.shape
    s1 = x.sum(dim=[1, 2, 3])
