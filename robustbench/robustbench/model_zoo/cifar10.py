...
class Hendrycks2020AugMixResNeXtNet(CifarResNeXt):
    ...
    def forward(self, x,return_feature=False):
        ...
        return super().forward(x,return_feature=return_feature)
...
class Hendrycks2019UsingNet(WideResNet):
    ...
    def forward(self, x,return_feature=False):
        x = 2. * x - 1.
        return super(Hendrycks2019UsingNet, self).forward(x,return_feature=return_feature)
...
class Kireev2021EffectivenessNet(PreActResNet):
    ...
    def forward(self, x,return_feature=False):
        x = (x - self.mu) / self.sigma
        return super(Kireev2021EffectivenessNet, self).forward(x,return_feature=return_feature)
...
class Diffenderfer2021CARD_Deck(nn.Module):
    ...
    def forward(self, x, return_feature=False):
        x = (x - self.mu) / self.sigma
        x_cl = x.clone()  # Clone to ensure x is not modified by inplace methods
        out_list = []
        feature_list =[]
        for i in range(self.num_cards):
            # Pass the flags directly to each model's forward function
            if return_feature:
                logits, features = self.models[i](x_cl, return_feature=return_feature)
                out_list.append(logits)
                feature_list.append(features)
            else:
                logits = self.models[i](x_cl)
                out = torch.softmax(logits, dim=1) 
                out_list.append(out)
        if return_feature:
            avg_logits = torch.mean(torch.stack(out_list), dim=0)
            avg_feature = torch.mean(torch.stack(feature_list), dim=0)
            # Return average logits and the last model's final feature output
            return avg_logits, avg_feature #features
        else:
            return torch.mean(torch.stack(out_list), dim=0)
    def get_fc(self):
        #fc = self.models[-1].fc
        #return fc.weight.cpu().detach().numpy(), fc.bias.cpu().detach().numpy()
        weight_shape = self.models[0].fc.weight.shape
        bias_shape = self.models[0].fc.bias.shape
        total_weight = torch.zeros(weight_shape, device=self.models[0].fc.weight.device)
        total_bias = torch.zeros(bias_shape, device=self.models[0].fc.bias.device)
        for model in self.models:
            total_weight += model.fc.weight
            total_bias += model.fc.bias
        avg_weight = (total_weight / len(self.models)).cpu().detach().numpy()
        avg_bias = (total_bias / len(self.models)).cpu().detach().numpy()
        return avg_weight, avg_bias

    def get_fc_layer(self):
        return self.models[-1].fc
...
class Diffenderfer2021CARD_Deck_Binary(torch.nn.Module):
    ...
    def get_fc_layer(self):
        # Return the fully connected layer from the last model only
        return self.models[-1].fc
