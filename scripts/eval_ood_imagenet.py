...
parser.add_argument('--aug_type', type=str, default="", help='data augmentation')
...
if os.path.isfile(
        os.path.join(root, 'postprocessors', f'{postprocessor_name}.pkl')) and not args.overwrite:
    with open(
...
if hasattr(evaluator.postprocessor, 'setup_flag'
    ...
    if not os.path.isfile(os.path.join(pp_save_root, pp_save_filename)):
...
if args.save_csv:
    ...
    if not os.path.isfile(
            os.path.join(saving_root,csv_filename )) or args.overwrite:
        metrics.to_csv(os.path.join(saving_root,csv_filename),
...
if args.save_score:
    ...
    with open(os.path.join(score_save_root,score_save_filename),
              'wb') as f:
