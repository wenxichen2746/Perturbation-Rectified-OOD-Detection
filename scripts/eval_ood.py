...
for subfolder in sorted(glob(os.path.join(root, 's*'))):
    ...
    net.load_state_dict(
        torch.load(os.path.join(subfolder, 'best.ckpt'), map_location='cpu',weights_only=True))
    net.cuda()
    net.eval()
...
if args.save_csv:
    ...
    df.to_csv(os.path.join(saving_root, csv_filename))
    print(f'result saved to{os.path.join(saving_root, csv_filename)}')
print(df)
